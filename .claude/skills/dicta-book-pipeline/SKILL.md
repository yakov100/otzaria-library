...
5. **מילה ראשונה של כל קטע מודגשת** — `תוספות א"נ...` ⇒ `<b>תוספות</b> א"נ...`.
6. **רמות יחסיות** — דף = רמה אחת מתחת לפרק; פרק = רמה אחת מתחת למסכת/חלק. אם פרק=`<h2>` אז דף=`<h3>`; אם פרק=`<h3>` אז דף=`<h4>`. בלי מספר רמה גלובלי קבוע.
...
2. **נקה `<h1>`** — הסר גרשיים (`"`/`”`/`’`).
3. **הסר `<big>`** מכותרות ומשורות דקורטיביות בודדות (לא קולופון סיום).
4. **נרמל כותרות `דף`** — `page-number --file <f> --style dot-colon` ו‑`replace-page-b` להמרת `עמוד ב`/גרש ל‑`.`/`:`.
5. **הדגש מילה ראשונה** — `emphasize-first --file <f>` (מוסיף `<b>` + פיסוק סוף).
6. **הפרד כותרות דבוקות לטקסט** — שורה שמתחילה `<h` ולא מסתיימת ב‑`>`: כותרת בשורה אחת, טקסט בשורה הבאה. אתר: `grep -nE '^<h[1-6]>' <f> | grep -vE '>[[:space:]]*$'`.
